...
flags.DEFINE_string('model_dir', 'expmnist/rescaleFalse/',
                    'Directory to write results and logs.')
flags.DEFINE_string('data_dir', 'image_data/',
                    'Directory to data np arrays.')
flags.DEFINE_integer('ckpt_step', 10, 'The step of the selected ckpt.')
flags.DEFINE_string('exp', 'mnist', 'cifar or fashion')
...
def main(unused_argv):
  ...
  if FLAGS.exp == 'fashion' or 'mnist':
    ...
    plt_file = os.path.join(
        out_dir, 'run%d_heatmap_mnist_p(x).pdf' % FLAGS.repeat_id)
    ...
    plt_file = os.path.join(
        out_dir, 'run%d_heatmap_mnist_LLR(x).pdf' % FLAGS.repeat_id)
    plot_heatmap(n, data, plt_file)
    ...
    plt_file = os.path.join(out_dir,
                            'run%d_heatmap_devanagari_p(x).pdf' % FLAGS.repeat_id)
    plot_heatmap(n, data, plt_file)
    ...
    plt_file = os.path.join(out_dir,
                            'run%d_heatmap_devanagari_LLR(x).pdf' % FLAGS.repeat_id)
    plot_heatmap(n, data, plt_file)

...
flags.DEFINE_integer('save_every', 10, 'Interval to save model steps.')
...
def main(unused_argv):
  ...
  params = {
      ...
      'n_dim': 28 if FLAGS.exp == 'fashion' or 'mnist' else 32,
      'n_channel': 1 if FLAGS.exp == 'fashion' or 'mnist' else 3,
      'exp': FLAGS.exp,
  ...
  tr_in_ds = datasets['tr_in'].map(lambda x: utils.image_preprocess_add_noise(
      x, params['mutation_rate'], "gaussian")).batch(

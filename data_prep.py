...
class mnist_data_prep:
    ...
    def __init__(self):
        ...
        images=read_images('../data/t10k-images-idx3-ubyte.gz')
        labels=read_labels('../data/t10k-labels-idx1-ubyte.gz')
        images=images*(1./255.)
...
class hindi_letters:
    ...
    def __init__(self):
        root_path = '../data/archive/nhcd/nhcd/'
        ...
        for i,folder in enumerate(to_process):
            ...
            labels_ = sorted(list(map(str, range(1, len(glob.glob(root_path + folder + '*')) + 1))))
            labels_ = [int(l)+(i*36) for l in labels_]
            #labels_ = glob.glob(root_path+folder+'*')
            #labels_ = [(l+'_{}'.format(i)).split('\\')[-1] for l in labels_]
            num = int(len(files_to_process)/len(labels_))
            labels_ = list(itertools.chain.from_iterable(itertools.repeat(t, num) for t in labels_))
